...
# datalab/training-data-analyst/courses/machine_learning/deepdive/09_sequence/labs/sinemodel/model.py
...
def linear_model(features, mode, params):
    ...
    # 선형으로 모델링 할 때는 활성화 함수을 사용하지 않는 dense layer하나만 깔아주면 된다.
    # 선형 모델이 구불부구불한 사인 함수를 모델링하기에 적합할까?
...
def dnn_model(features, mode, params):
    ...
    # 레이어를 추가해주고 히든 레이어의 경우는 비선형의 활성화 함수를 추가한다.
    # 선형모델보다 예측력이 좋아지는 것을 확인하자.
...
def cnn_model(features, mode, params):
    ...
    # 스포일러: CNN은 DNN에 비해 Loss가 개선되지 않는다. 왜 그럴까?
     
    # single drop anormly detector를 생각하면서 모델링
    # two drops를 detect하기 위해서는? 좀 더 긴 filter를 사용하거나 레이어를 쌓아준다.
    X = tf.reshape(features[TIMESERIES_COL],
                   [-1, N_INPUTS, 1])  # as a 1D "sequence" with only one time-series observation (height)
    
    # 필터수를 너무 많이 하면 overfitting되기 때문에 절반 정도로 설정하였다. 바꿀 수 있는 하이퍼 파라미터이다.
    c1 = tf.layers.conv1d(X, filters=N_INPUTS // 2,
                          kernel_size=3, strides=1,
                          padding='same', activation=tf.nn.relu)

    # max pooling layer는 tensor size를 줄여주면서 spatial invariance를 제공해준다.
    p1 = tf.layers.max_pooling1d(c1, pool_size=2, strides=2)

    # c1, p1 작업과 동일한 작업의 반복.  p1을 받아서 c2를 쌓고 c2를 p2에 넘겨 maxpooling 해 준다. 
...
def rnn2_model(features, mode, params):
    ...
    # 2. configure the RNN
    # cell 사이즈를 다르게 하면서 테스트해보기 위해서 cell1은 작게, cell2는 크게 해 본다.
...
def rnnN_model(features, mode, params):
    ...
    # rnn2를 수정하되 Multiple loss를 계산해서 multiple prediction을 평균하는 방법을 생각해보자.
    ...
    # for each time step 
    # rnn2에서는 state[1]을 넘겨주던 부분을 49개의 outputs로 바꿔준다.
    ...
    # 49 predictions! loss average를 구하는 부분은 compute_errors쪽에 구현 
